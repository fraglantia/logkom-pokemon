...
w :- 
	retract(tokemonExpUp(Id,ExpUp)), stat_tokemon(Id,Name,H,Level,Exp,ExpMax), ExpNew is (Exp + ExpUp),(\+ isLevelUp(ExpNew,ExpMax)), 
...
w :- 
	retract(tokemonExpUp(Id,ExpUp)),(retract(stat_tokemon(Id,Name,H,Level,Exp,ExpMax)), ExpTemp is (Exp + ExpUp),
	...
	asserta(stat_tokemon(Id,Name,Hup,LevelUp,ExpNew,ExpMaxNew))).
w :-
	pemain(Name, L, X, Y, Map),
	Ynext is (Y-1),
...
checkIfEnemyDead(Id) :- 
	...
	makeCanCapture(Id), ExpUp is Level*50, asserta(tokemonExpUp(IdUp,ExpUp)), !.
